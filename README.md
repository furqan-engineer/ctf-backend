...
4. Set `GEMINI_API_KEY` (and any other vars from `.env.example`) in the
   Vercel dashboard.
5. `vercel.json` already sets `maxDuration: 300` for this function so a slower
   analysis (binwalk extraction, foremost carving) has room to finish inline
   before the request times out. If your plan caps duration lower than that,
   heavy pipelines may get cut off — Railway doesn't have this constraint
   since it's a normal long-running container.
